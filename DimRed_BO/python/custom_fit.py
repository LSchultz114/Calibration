...
def fit_gpytorch_torch(
    ...
    Example:
        ...
        >>> mll.eval()
    """
    optim_options = {"maxiter": 100, "disp": False, "lr": 0.05}
    ...
    if exclude is not None:
        ...
        mll_names=[p_name for p_name, t in mll.named_parameters()]
    ...
    if hasattr(mll, "named_parameters_and_constraints"):
        for ml_param in mll_names:
            if mll.constraint_for_parameter_name(ml_param) is not None and not mll.constraint_for_parameter_name(ml_param).enforced:
                bounds_[ml_param] = mll.constraint_for_parameter_name(ml_param).lower_bound, mll.constraint_for_parameter_name(ml_param).upper_bound
